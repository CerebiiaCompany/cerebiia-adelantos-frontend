...
export interface EmployeeUnifiedAuditTableProps {
...
export function EmployeeUnifiedAuditTable({
  records,
  isLoading,
  isError,
  onRetry,
}: EmployeeUnifiedAuditTableProps) {
  const [category, setCategory] = useState<EmployeeAuditCategory>("todos");
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Barra de Filtros: Buscador + Filtro por Fecha + Selector de categoría */}
      <div
        className={cn(
      ...
      >
        <div className="flex flex-1 flex-col gap-2 sm:flex-row sm:items-center">
          <div className="relative max-w-sm flex-1">
            ...
            <Input
              placeholder="Buscar por evento, monto, actor..."
              value={search}
        ...
        <div className="inline-flex flex-wrap rounded-xl border border-border/80 bg-secondary/30 p-1 text-xs">
          ...
          <button
            type="button"
            onClick={() => setCategory("solicitudes")}
            className={cn(
              "rounded-lg px-3 py-1.5 font-medium transition-all",
          ...
          >
            Adelantos ({records.filter((r) => r.category === "solicitudes").length})
          </button>
          <button
            type="button"
            onClick={() => setCategory("cupo")}
            className={cn(
              "rounded-lg px-3 py-1.5 font-medium transition-all",
              category === "cupo"
                ? "bg-background font-semibold text-foreground shadow-sm"
          ...
          >
            Cupo ({records.filter((r) => r.category === "cupo").length})
          </button>
          <button
            type="button"
          ...
          >
            Datos personales ({records.filter((r) => r.category === "perfil").length})
          </button>
        </div>
      </div>
      ...
      {/* Modal de Comprobante / Evidencia */}
...
function AuditRowItem({
  ...
  record: EmployeeUnifiedAuditRecord;
  isExpanded: boolean;
  onToggle: () => void;
                ...
                <table className="w-full text-sm">
                  <thead className="bg-muted/40 text-xs uppercase tracking-wide text-muted-foreground">
                    <tr>
                      ...
                      <th className="px-3 py-2 font-medium">Antes de liberar</th>
                      <th className="px-3 py-2 font-medium">Después de liberar</th>
                    </tr>
                  </thead>
                  <tbody>
                    <tr className="border-t border-border/40">
                      <td className="px-3 py-2 font-medium text-foreground">
                        Cupo disponible para adelantar
                      </td>
                      <td className="px-3 py-2 text-muted-foreground tabular-nums">
                        {formatCOP(record.cupoAnterior ?? 0)}
                      </td>
                      <td className="px-3 py-2 font-semibold text-emerald-600 dark:text-emerald-400 tabular-nums">
                        {formatCOP(record.cupoNuevo ?? 0)}
                      </td>
                    </tr>
                    <tr className="border-t border-border/40">
                      <td className="px-3 py-2 font-medium text-foreground">
                        Cuota liquidada por Super Admin
                      </td>
                      <td className="px-3 py-2 text-muted-foreground">
                        Pendiente
                      </td>
                      <td className="px-3 py-2 text-foreground font-semibold tabular-nums">
                        +{formatCOP(record.amount ?? 0)} (Cuota #{record.currentInstallment} de {record.installments})
                      </td>
                    </tr>
                  </tbody>
                </table>
